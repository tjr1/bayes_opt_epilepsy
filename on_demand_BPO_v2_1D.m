function varargout = on_demand_BPO_v2_1D(varargin)
% ON_DEMAND_BPO_V2_1D MATLAB code for on_demand_BPO_v2_1D.fig
%      ON_DEMAND_BPO_V2_1D, by itself, creates a new ON_DEMAND_BPO_V2_1D or raises the existing
%      singleton*.
%
%      H = ON_DEMAND_BPO_V2_1D returns the handle to a new ON_DEMAND_BPO_V2_1D or the handle to
%      the existing singleton*.
%
%      ON_DEMAND_BPO_V2_1D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ON_DEMAND_BPO_V2_1D.M with the given input arguments.
%
%      ON_DEMAND_BPO_V2_1D('Property','Value',...) creates a new ON_DEMAND_BPO_V2_1D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before on_demand_BPO_v2_1D_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to on_demand_BPO_v2_1D_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help on_demand_BPO_v2_1D

% Last Modified by GUIDE v2.5 28-May-2018 14:10:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @on_demand_BPO_v2_1D_OpeningFcn, ...
                   'gui_OutputFcn',  @on_demand_BPO_v2_1D_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before on_demand_BPO_v2_1D is made visible.
function on_demand_BPO_v2_1D_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to on_demand_BPO_v2_1D (see VARARGIN)

% Choose default command line output for on_demand_BPO_v2_1D
...
% UIWAIT makes on_demand_BPO_v2_1D wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = on_demand_BPO_v2_1D_OutputFcn(hObject, eventdata, handles) 
...
freq = [1, 2, 5, 100, 105, 110, 150, 151, 195, 196]';
InitialObjective = 20*(1.2+cos(2*pi*freq/max(freq(:)))+.25*randn(size(freq)));
InitialX = table(freq);
...
parfeval(@BO_wrapper_1D,0,opt_freq, InitialX, InitialObjective, q{1,1});
parfeval(@BO_wrapper_1D,0,opt_freq, InitialX, InitialObjective, q{1,2});
...
%         amp = stim_amp(1:Seizure_Count(1,i_ch_out),i_ch_out);
        ...
        parfeval(@BO_wrapper_1D,0,opt_freq, InitialX, InitialObjective, q{1,i_ch_out});
...
res = bayesopt(@place_holder_fcn,[opt_freq],'InitialX',InitialX,'InitialObjective',InitialObjective, 'MaxObjectiveEvaluations', 1, 'PlotFcn', [], 'AcquisitionFunctionName', 'expected-improvement-plus', 'ExplorationRatio', .4);
pause(3)
send(que,res)
...
if any(stim_remaining>0) 
    for i_ch_out = 1:n_ch_out
        if stim_remaining(i_ch_out)>0
            ...
            plot(handles.A_NextStim,t(plot_vec),data((plot_vec ),i_ch_out)+(i_ch_out-1)*10)
            hold on
